...
def fetch_espn():
    ...
    headers={"User-Agent":"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36","Accept":"application/json","Referer":"https://africa.espn.com/football/fixtures"}
    ...
    def get_json(url):
        ...
        # 2. via proxy - THIS FIXES RENDER 403
        try:
    ...
    for date_str in dates:
        ...
        # ALL
        data=get_json(f"https://site.api.espn.com/apis/site/v2/sports/soccer/all/scoreboard?dates={date_str}&limit=400")
        if data:
        ...
        # LEAGUE LOOP - guaranteed to work
        if len(games)<10:
            for code in LEAGUES:
                if len(games)>40: break
                data=get_json(f"https://site.api.espn.com/apis/site/v2/sports/soccer/{code}/scoreboard?dates={date_str}")
                if not data: continue
                for ev in data.get("events",[])[:3]:
                    comp=(ev.get("competitions") or [{}])[0]; cs=comp.get("competitors") or []
                    if len(cs)<2: continue
                    home=next((x for x in cs if x.get("homeAway")=="home"),cs[0]); away=next((x for x in cs if x.get("homeAway")=="away"),cs[1])
                    lname=data.get("leagues",[{}])[0].get("name") if data.get("leagues") else code; country,flag=classify_country(lname)
                    st=(comp.get("status",{}).get("type",{}) or {}); games.append({"league":lname,"leagueName":lname,"country":country,"flag":flag,"home":(home.get("team",{}).get("displayName") or "Home")[:40],"away":(away.get("team",{}).get("displayName") or "Away")[:40],"score":st.get("shortDetail") or "TBD","live":st.get("state")=="in"})
